...
func TestResolverUsesNativeMetricsForImplicitProfiles(t *testing.T) {
	resolver := NewResolver(stubGeoResolver{
		geo: GeoData{
			CountryCode: "ES",
			Locale:      "es-ES",
	...
	if profile.ScreenWidth != 1920 || profile.ScreenHeight != 1080 {
		t.Fatalf("expected native screen metrics, got %dx%d", profile.ScreenWidth, profile.ScreenHeight)
	}
	if profile.WebGLRenderer != "Intel(R) Iris(R) Xe Graphics" {
		t.Fatalf("expected native webgl renderer, got %q", profile.WebGLRenderer)
	}
	if profile.DeviceScaleFactor != 1.25 {
		t.Fatalf("expected native device scale factor, got %v", profile.DeviceScaleFactor)
	}
}
